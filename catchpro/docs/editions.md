...
| `insungFree` | 인성 CatchPro Free | `com.catchpro.insung.free` | 무료 안내/체험 신청판. 권한 확인과 Pro 체험 신청만 제공하며, 오더 조건과 오더확정 보조는 잠근다. |
...
- `오더 조건`: 고객 배포판에서는 `insungPro`에서만 실제 설정 화면으로 제공한다. `insungFree`는 하단 탭에서 오더 조건을 빼고 `Pro 체험하기` 탭에서 첫 달 무료체험과 월 9,900원 구독 신청 흐름으로 안내한다.
...
현재 Pro 분리는 컴파일 타임 APK 분리 구조다. 초기 운영은 홈페이지 신청, 상담 승인, 한 달 무료체험, 월 9,900원 구독 전환을 수동으로 관리한다. Google Play 결제나 자동결제를 붙이기 전까지는 서버 라이선스 검증으로 Pro APK 사용 권한을 제한한다.
